            ...
            <div className='flex flex-col gap-6'>
                <div className='flex items-center justify-between bg-[#0f0f11] border border-white/10 rounded-2xl px-6 py-4'>
                    <div className='flex items-center gap-3'>
                        <span className='w-9 h-9 rounded-xl bg-amber-400/10 border
                        border-amber-400/20 flex items-center justify-center text-base shrink-0'>
                            {ONBOARDING_ROLES.find((r)=> r.value === role)?.icon}
                        </span>
                        <div>
                            <p className='text-sm font-medium text-stone-200'>{ONBOARDING_ROLES.find((r)=> r.value === role)?.title}</p>
                            <p className='text-xs text-stone-600 mt-0.5'>Selected role</p>
                        </div>
                    </div>
                    <Button variant='outline' size='sm' onClick={()=> setRole(null)}>Change</Button>
                </div>
                ...
                {/* Interviewer form */}
                {
                    role === "INTERVIEWER" && (
                        <div className='bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-6'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                                {
                                    YEARS_OPTIONS.map((opt) =>(
                                        ...
                                        key={opt.value} 
                                        type = "button"
                                        onClick={()=>setForm((p)=>({...p,yearExp:opt.value}))}
                                        className={`text-xs px-4 py-2 rounded-lg border ${
                                            form.yearExp === opt.value?"border-amber-400/40 bg-amber-400/10 text-amber-400"
                            ...
                            rows={4}
                            maxLength={300}
                            placeholder="Tell interiviewees about your background, what you specilise in, and what they can expect from a session with you."
                            value={form.bio}
