    ...
    <div className='flex flex-col gap-8 '>
        <div className='flex flex-col gap-4'>
            <div className='relative max-w-sm'>
                <Search
                size={14}
                className='absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-600 pointer-event-none'
                />
                <Input
                value={search}
        ...
        {
            filtered.length === 0 ?( 
                // when interview no found
            <div className='py-20 text-center'>
                <p>No interviewers match your filters.</p>
                <Button onClick={()=>{setActiveCategory(null); setSearch("")}} variant='link'>
                    Clear filters
                </Button>
            </div>
             ) : (
             <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {filtered.map((interviewers)=>(
                    <InterviewerCard key={interviewers.id} interviewers={interviewers}/>
                ))}
             </div>
        )}
    </div>
